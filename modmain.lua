...
--[[
    ...
    1.【祈雨祭】（1）修复了携带[月藏宝匣]上下线导致玩家重选角色的问题；（2）月藏宝匣不再能放入宝石；（3）月藏宝匣的样子会根据升级原物的种类变化了
    2.【尘市蜃楼】（1）优化了幻化逻辑，不会出现解除幻化时有贴图没有清除的问题了；（2）新增第四格幻化栏-高位身体栏(以前是头部栏、身体栏和手持栏)
    3.【无人认领】[臭臭自动清理]选项默认为“否”了
]]
