...
local PerennialCrop2 = Class(function(self, inst)
	...
	self.growthmults = { 1, 1, 1, 0 } --四个季节的生长速度(大于1为快，小于1为慢) --undo
	...
	self.fn_timemult = nil --生长速度额外修正：fn(self)
...
function PerennialCrop2:SetPauseReason(key, value) --改变一个暂停生长原因
	if value == nil then --减
...
function PerennialCrop2:UpdateTimeMult() --更新生长速度
	if self.isrotten or self.stage == self.stage_max then --枯萎恢复与过熟时间不受影响
		...
		if TheWorld.state.season == "winter" then
			self.time_mult = self.growthmults[4]
		elseif TheWorld.state.season == "summer" then
			self.time_mult = self.growthmults[2]
		elseif TheWorld.state.season == "spring" then
			self.time_mult = self.growthmults[1]
		else --默认为秋，其他mod的特殊季节默认都为秋季
			self.time_mult = self.growthmults[3]
		end
		if self.fn_timemult ~= nil then
			self.fn_timemult(self)
		end
		if self.time_mult == nil or self.time_mult <= 0 then
			self.time_mult = nil
			return
		end
		--浇水、施肥、照顾，能加快生长
		...
		self.time_mult = self.time_mult * mulmul
	end
end

function PerennialCrop2:TryDoGrowth() --循环生长
	if self.time_grow == nil or self.time_grow <= 0 then
		return
	end
...
function PerennialCrop2:TimePassed(time)
...
function PerennialCrop2:StartGrowing() --尝试生长
	...
	if self.task_grow == nil and not self.inst:IsAsleep() then
		...
		self.task_grow = self.inst:DoPeriodicTask(15+math.random()*9, function(inst, self)
			if self.time_start ~= nil then
				self:TimePassed(GetTime() - self.time_start)
			end
			self.time_start = GetTime()
		end, 0, self)
	end
