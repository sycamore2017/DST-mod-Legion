...
local function OnWorkedFinish(inst, worker)
...
function PerennialCrop:SetUp(data)
	...
	inst.components.workable:SetOnFinishCallback(OnWorkedFinish)
...
function PerennialCrop:CostNutrition() --养料水分索取
...
function PerennialCrop:GenerateLoot(doer, ispicked, isburnt) --生成收获物
	...
	if not ispicked then --非采集时，多半是破坏
		local soil = SpawnPrefab("siving_soil_item")
		if soil ~= nil then
