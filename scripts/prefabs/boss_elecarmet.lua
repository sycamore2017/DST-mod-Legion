...
local function SpawnLightning(x, y, z)
    ...
    local ents = TheSim:FindEntities(x, y, z, 30, nil, { "ghost", "playerghost", "INLIMBO" }, { "lightningrod", "lightningtarget" })
    for k, v in pairs(ents) do
        local visrod = v:HasTag("lightningrod")
        local vpos = v:GetPosition()
        local vdistsq = distsq(x, z, vpos.x, vpos.z)
    ...
    if isrod then
        ...
        ents = TheSim:FindEntities(x, y, z, 3, nil, { "player", "INLIMBO", "electrified" })
        for k, v in pairs(ents) do
    ...
    SpawnPrefab("fimbul_lightning").Transform:SetPosition(x, y, z)
end
